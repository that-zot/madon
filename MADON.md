...
- **Base:** Mastodon `v4.6.5` — <https://github.com/mastodon/mastodon/tree/v4.6.5>
- **Our changes:** everything in `git diff v4.6.5..madon` — a few
...
fork is regenerated as *upstream `v4.6.5` + our overlays* and force-pushed by a
sync script. Because it is generated, **issues and pull requests here may go
unmonitored**. A per-version `madon-v4.6.5` tag marks each published Mastodon
version. This repo is a 2-commit snapshot (upstream `v4.6.5` + overlays), not a
